...
- 의사결정형 질문은 RAG 답변 API 내부에서 후보 분석과 점수화를 수행
...
7. 검색된 근거 문서를 기반으로 LLM 답변을 생성하거나, 의사결정형 질문은 서버가 후보 분석과 점수화를 직접 수행합니다.
...
의사결정형 질문은 별도 API가 아니라 기존 RAG 답변 API에서 자동 처리합니다.

예시:

```text
서울에서 4억 미만으로 살만한 아파트 추천해줘
대치동과 잠실 20평대 비교해줘
실거주 기준으로 어디가 나아?
```

서버는 실거래 후보를 단지/동 단위로 집계하고 다음 기준으로 점수화합니다.
...
이 결과는 기존 `/api/v1/rag/ask`와 `/api/v1/rag/ask/stream` 응답으로 반환되므로, 프론트는 하나의 AI API만 사용하면 됩니다.
...
| POST | `/api/v1/rag/ask` | RAG 답변 및 의사결정형 후보 분석 |
| POST | `/api/v1/rag/ask/stream` | SSE 기반 RAG 답변 및 의사결정형 후보 분석 |
...
의사결정 분석을 별도 API로 분리하면 프론트가 질문 유형별로 API를 선택해야 하는 문제가 있습니다. 이를 피하기 위해 기존 RAG 답변 API 내부에서 질문 유형을 판별하고, 필요한 경우 decision engine을 실행하도록 통합했습니다.
